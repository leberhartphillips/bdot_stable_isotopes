...
if (!all(subset_check$rings_match_frozen_subset)) {
  stop("The AU vs non_AU extension tables do not all match the frozen 56-ring subset.")
}

expected_rings <- sort(unique(matched_tbl$ring))

if (!identical(sort(unique(breast_tbl$ring)), expected_rings)) {
  stop("Breast feature table does not match the frozen AU vs non_AU ring set.")
}

if (!identical(sort(unique(paired_tbl$ring)), expected_rings)) {
  stop("Paired-contrast feature table does not match the frozen AU vs non_AU ring set.")
}
...
write_csv(
  ...
  file.path(derived_dir, "model_stage6_au_non_au_config.csv")
)
...
candidate_defs <- tibble(
  ...
  candidate_label = c(
    "AU vs non_AU A: Breast-only C/N/H",
    "AU vs non_AU B: Breast C/N/H + paired contrasts",
    "Benchmark: R2 S2 collapsed to AU vs non_AU",
    "Benchmark: R2 C collapsed to AU vs non_AU"
  ),
  model_type = c(
    "ridge_binomial",
    "ridge_binomial",
  ...
  caution_level = c(
    "breast_cnh_binary_baseline",
    "breast_cnh_plus_paired_h_high_caution",
    "collapsed_from_siteaware_staged_rung2",
    "collapsed_from_siteaware_direct_rung2"
  ),
  source_table = c(
...
write_csv(
  candidate_defs,
  file.path(derived_dir, "model_stage6_au_non_au_candidate_definitions.csv")
)

direct_candidates <- list(
  list(
    candidate_id = "au_non_au_a_breast_cnh",
    candidate_label = "AU vs non_AU A: Breast-only C/N/H",
    model_type = "ridge_binomial",
    candidate_group = "direct_binary_baseline",
    caution_level = "breast_cnh_binary_baseline",
    predictors = c(
  ...
  list(
    candidate_id = "au_non_au_b_breast_cnh_plus_contrast_cnh",
    candidate_label = "AU vs non_AU B: Breast C/N/H + paired contrasts",
    model_type = "ridge_binomial",
    candidate_group = "direct_binary_extension",
    caution_level = "breast_cnh_plus_paired_h_high_caution",
    predictors = c(
      "normalised_d13c_Breast",
      "normalised_d15n_Breast",
      "normalised_d2h_Breast",
...
write_csv(
  all_folds,
  file.path(derived_dir, "model_stage6_au_non_au_outer_folds.csv")
)

write_csv(
  all_split_metrics,
  file.path(derived_dir, "model_stage6_au_non_au_split_metrics.csv")
)

write_csv(
  all_predictions,
  file.path(derived_dir, "model_stage6_au_non_au_assessment_predictions.csv")
)
...
write_csv(
  calibration_summary,
  file.path(derived_dir, "model_stage6_au_non_au_calibration_summary.csv")
)

write_csv(
  comparison_summary,
  file.path(derived_dir, "model_stage6_au_non_au_vs_benchmarks.csv")
)
...
summary_lines <- c(
  "# AU vs non_AU exploratory extension",
  "",
  paste0(
  ...
  "It is reported for direct and collapsed benchmarks on the same footing, but it is not a Stage 4-style uncertainty-aware operational rule."
)

writeLines(
  summary_lines,
  file.path(derived_dir, "model_stage6_au_non_au_summary.md")
)
